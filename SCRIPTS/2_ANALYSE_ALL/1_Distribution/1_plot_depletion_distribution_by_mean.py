...
PLOT_PER_IPEI = True  # True = frequency is unique IPEIs, False = daily samples
...
# -------------------------------------------------
# OPTIONAL: PER-IPEI AGGREGATION
# -------------------------------------------------
if PLOT_PER_IPEI:
    plot_df = (
        df.groupby("ipei", dropna=False)[RATE_COLUMN]
        .mean()
        .reset_index()
    )
    frequency_label = "Unique IPEI Count"
    sample_label = "IPEIs analysed"
else:
    plot_df = df.copy()
    frequency_label = "Daily Sample Count"
    sample_label = "Daily samples analysed"
...
# SUMMARY
# -------------------------------------------------
mean_rate = plot_df[RATE_COLUMN].mean()
median_rate = plot_df[RATE_COLUMN].median()
std_rate = plot_df[RATE_COLUMN].std()

data_min = plot_df[RATE_COLUMN].min()
data_max = plot_df[RATE_COLUMN].max()
bin_width = (data_max - data_min) / BIN_COUNT

pass_count = (plot_df[RATE_COLUMN] < RATED_DEPLETION).sum()
fail_count = (plot_df[RATE_COLUMN] >= RATED_DEPLETION).sum()
...
print(f"Raw depletion samples : {raw_data_count}")

if PLOT_PER_IPEI:
    print(f"IPEIs plotted         : {len(plot_df)}")
else:
    print(f"Daily samples plotted : {len(plot_df)}")
...
#plt.axvline(mean_rate, linestyle="--", label=f"Mean = {mean_rate:.2f}%/day")
#plt.axvline(median_rate, linestyle=":", label=f"Median = {median_rate:.2f}%/day")
plt.axvline(
    RATED_DEPLETION,
    linestyle=":",
    linewidth=2,
...
if PLOT_PER_IPEI:
    plt.title("Per-IPEI Distribution of Mean Battery Depletion Rates")
else:
    plt.title("Daily Distribution of Battery Depletion Rates")
